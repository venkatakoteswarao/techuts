...
class TechUpdate(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    title = db.Column(db.String(255), nullable=False)  # Ensure 'title' column is defined
    description = db.Column(db.Text)
...
def home():
    ...
    return render_template('user/index.html', carousels=carousels)
...
def admin_login():
    if request.method == 'POST':
        ...
        if username == 'gopiSankarnelluRI' and password == 'gopicseaiml.2K22':
            session['admin'] = True
            return redirect(url_for('admin_dashboard'))  # Redirect to the correct dashboard route
...
def admin_dashboard():
    if not session.get('admin'):
        return redirect(url_for('admin_login'))  # If not logged in, redirect to login
    carousels = Carousel.query.all()
    posts = InstagramPost.query.all()
    updates = TechUpdate.query.all()
    about_content = About.query.first()
    return render_template('admin/admin1.html', carousels=carousels, posts=posts, updates=updates, about=about_content)
